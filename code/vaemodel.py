...
def decoder(sampled_z, keep_prob):
    with tf.variable_scope("decoder", reuse=None):
        ...
        x = tf.layers.conv2d_transpose(x, filters=16, kernel_size=3, strides=1, padding='same', activation=tf.nn.softmax)
        x = tf.nn.dropout(x, keep_prob)
        x = tf.layers.conv2d_transpose(x, filters=16, kernel_size=3, strides=1, padding='same', activation=tf.nn.softmax)
        x = tf.nn.dropout(x, keep_prob)
        x = tf.layers.conv2d_transpose(x, filters=16, kernel_size=3, strides=1, padding='same', activation=tf.nn.softmax)
...
for i in range(num_epochs):
    ...
    if not i % 2:
        ...
        test_acc = tf.metrics.accuracy(Y_test, Y_pred_test)
        print(test_acc)
        #print("Test accuracy = %s" % str(test_acc))
        Y_pred_train = sess.run(img, feed_dict={X_in: X_train, keep_prob: 1})
        train_acc = tf.metrics.accuracy(Y_train, Y_pred_train)
        print(train_acc)
        #print("Test accuracy = %s" % str(train_acc))
    """
